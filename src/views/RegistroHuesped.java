...
*/
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (true) {
					...
					;
					System.out.println("huesped guardado con exito");
				} else {
	...
	private void guardarHuesped() {
		...
		if(txtNombre.getText() != "" && txtApellido.getText() != "" && txtFechaN.getDate() != null && txtTelefono.getText() !="" ) {
			Date fechaN = java.sql.Date.valueOf(((JTextField) txtFechaN.getDateEditor().getUiComponent()).getText());
			int valor = Integer.parseInt(txtNreserva.getText());
